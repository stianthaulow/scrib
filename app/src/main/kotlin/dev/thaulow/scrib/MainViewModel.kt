...
class MainViewModel(
    ...
    fun handleSharedText(text: String, key: String) {
    ...
    fun moveCursorToLine(delta: Int, layout: TextLayoutResult) {
    ...
    fun flushPendingSave() {
        ...
        runBlocking(Dispatchers.IO) { persist() }
    ...
    private fun persist() {
        runCatching { noteRepo.save(value.text) }
        runCatching { undoRepo.save(UndoStackDto(entries.toList(), index)) }
    }

    private fun isShareHandled(key: String): Boolean =
        savedStateHandle.get<String>(LAST_HANDLED_SHARE_KEY) == key
